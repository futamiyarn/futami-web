...
<script lang="ts">
  ...
  onMount(() => {
    ...
    window.addEventListener("flamethrower:router:fetch", () => {
      ...
      logoElement.style.setProperty("--round-size", "0px");
    });

    window.addEventListener("flamethrower:router:fetch-progress", (e) => {
      const { detail } = e as Event & { detail: any };

      logoElement.style.setProperty(
        "--round-size",
        `${(detail.progress * 48) / 100}px`,
      );
    });
    ...
    window.addEventListener("flamethrower:router:end", () => {
      ...
      logoElement.style.setProperty("--round-size", `48px`);
    });
  });
</script>
...
<!-- <style lang="scss">
  @use "../../styles/components/logo-ani.scss";
</style> -->
